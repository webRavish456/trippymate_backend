...
export const AddPayment = async (req, res) => {
  try {
    const { vendorId, bookingId, amount, paymentMethod, transactionId, notes, status } = req.body;

    if (!vendorId || !bookingId || !amount) {
      return res.status(400).json({
        status: false,
        message: "Vendor ID, Booking ID, and Amount are required"
      });
    }
    ...
    const payment = await VendorPayment.findById(newPayment._id)
...
export const GetAllPayments = async (req, res) => {
  try {
    const { vendorId, bookingId, status, page = 1, limit = 10 } = req.query;
    const query = {};
...
export const DeletePayment = async (req, res) => {
  try {
    ...
    const payment = await VendorPayment.findByIdAndDelete(id);
